...
const Alert: React.FC<AlertProps> = ({
  ...
  const config: Record<AlertType, { bgColor: string; textColor: string; icon: JSX.Element }> = {
    success: {
    ...
    warning: {
      bgColor: 'bg-warning-100 border-warning-500',
  ...
  return (
    <motion.div
      ...
      className={`border-l-4 p-4 rounded-r-md shadow-sm ${config[type].bgColor} ${className}`}
      role="alert"
    >
      <div className="flex items-start">
        ...
        <div className="ml-3 flex-1">
          {title && (
            <h3 className={`text-sm font-medium ${config[type].textColor}`}>
              {title}
            </h3>
          )}
          <div className={`text-sm ${config[type].textColor} mt-1`}>
            {children}
          </div>
        </div>
        {onDismiss && (
          <button
            type="button"
            onClick={onDismiss}
            className={`ml-auto -mx-1.5 -my-1.5 ${config[type].textColor} p-1.5 hover:bg-white hover:bg-opacity-20 rounded-md focus:outline-none focus:ring-2 focus:ring-primary-500`}
          >
